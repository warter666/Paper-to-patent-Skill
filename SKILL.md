...
2. **Closest prior technology before claim wording**: identify and verify the most relevant prior technology where external search is requested or available. Never invent a citation, publication number, date, or disclosed feature. If verification is unavailable, preserve the uncertainty rather than filling it.
   - For the Background section, prioritize patent documents published within the **two years immediately preceding the filing/application date supplied for the task**. “Recent” means this explicit two-year window, not merely a recent search result.
   - Do not list patents independently. Build a chronological/technical progression: **existing solution → targeted improvement → newly exposed limitation/problem → further improvement → remaining problem → the problem addressed by the present invention**.
   - Each cited patent must actually disclose the technical point attributed to it. Do not invent a relationship of “improvement over” between documents unless the document content supports that relationship.
   - If fewer than the required number of relevant patents can be verified in the two-year window, explicitly state that the recent-patent evidence is insufficient instead of silently substituting older documents.
   - The final paragraph of Background must naturally connect the accumulated remaining problems to the technical problem solved by the present invention, without prematurely revealing unsupported details of the invention.
